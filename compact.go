...
func (db *DB) mergeFiles() error {
	...
	for _, filename := range matched {
		println(filename)
		// TODO: ignore the active datafile.
		if filename == filepath.Join(db.path, fmt.Sprintf("%10d%s", db.activeFileId, dataFileExt)) {
			continue
		}
	...
	return writeMergeFileAndHint(mergedFilename, alive)
}
...
func readDataFile(filename string) ([]*kvEntry, error) {
	...
	for pos >= fi.Size() {
		...
		entry, err3 := newEntryFromHeader(header)
		if err3 != nil {
			return nil, err3
		}
...
func writeMergeFileAndHint(filename string, aliveEntries map[string]*kvEntry) error {
	// tmpFileId := db.activeFileId + 1
	// tmpFilename := fmt.Sprintf("%10d%s", tmpFileId, dataFileExt)
	tmpFile, err := os.OpenFile(filename, os.O_CREATE|os.O_RDWR, 0666)
	if err != nil {
		return err
	}
